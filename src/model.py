...
class InteractModel(nn.Module):
    ...
    def __init__(self, num_property_dim=5, cat_property_dim=3, tweet_dim=768,
        ...
        self.Model_2 = RespectiveLayer(in_channels_for_graph=hidden_dim, in_channels_for_text=hidden_dim, out_channels=output_dim, attention_dim=attention_dim, graph_num_heads=graph_num_heads, dropout=dropout, device=self.device)
    ...
    def forward(self, text, user_neighbor_index, num_feature, cat_feature, tweet_feature, des_feature, edge_index):
        ...
        tweet_feature = self.dropout(self.relu(self.tweet_linear(tweet_feature)))
        des_feature = self.dropout(self.relu(self.des_linear(des_feature)))
...
class RespectiveLayer(nn.Module):
    ...
    attention_dim=6, graph_num_heads=4, text_num_heads=4, dropout=0.5, device='cuda:4'):
        ...
        self.LModel = LModel(embed_dim=in_channels_for_text, num_heads=text_num_heads, dropout=dropout)
    ...
    def forward(self, user_text, user_neighbor_index, all_user_feature, edge_index):
        ...
        for _, _, adj in subgraph_loader:
            ...
            all_user_feature = self.dropout(self.relu(self.GCN(all_user_feature, index)))
...
class InteractLayer(nn.Module): 
    ...
    def forward(self, text, all_user_feature, user_neighbor_index):
        ...
        graph_tmp = self.linear_graph(graph_ini)
